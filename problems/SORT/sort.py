...
def exhaustive_reversal_distance(sequence, best=None):
    ...
    for leftbkpt, rightbkpt in moves:
        ...
        dist, path = exhaustive_reversal_distance(newsequence, best-1)
        if dist < best:
            best = dist
            bestpath = path
            move = [(bkpts[leftbkpt], bkpts[rightbkpt]-1)]
        if best <= (n+1)//2:
            break

    return 1 + best, move + bestpath
...
if __name__ == "__main__":
    ...
    with args.infile as f:
        ...
        for line1, line2 in zip(lines[::2], lines[1::2]):
            ...
            dist, path = reversal_distance(perm1, perm2)
