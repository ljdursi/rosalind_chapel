module SORT {
  ...
  proc exhaustive_reversal_distance(sequence, best) : (int, list(2*int)) {
    var bkpts = breakpoints(sequence);
    var move : 2*int = (-1, -1);
    var bestpath : list(2*int);
    ...
    for (leftbp, rightbp) in moves {
      ...
      const (dist, path) = exhaustive_reversal_distance(newsequence, best-1);
  ...
  proc main() {
    try! {
      ...
      for (line1, line2) in zip(lines[1.. by 3], lines[2.. by 3]) {
        ...
        const dist : int;
        const path : list(2*int);
        (dist, path) = exhaustive_reversal_distance(extended_perm, n+1);
        writeln(dist);
        for move in path do
          writeln(move[1]-1, " ", move[2]-2);
      }
    }
  }
}
